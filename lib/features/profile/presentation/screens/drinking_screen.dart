...
class DrinkingScreen extends ConsumerStatefulWidget {
  ...
  const DrinkingScreen({super.key, this.isEditMode = false});
...
class _DrinkingScreenState extends ConsumerState<DrinkingScreen> {
  ...
  void initState() {
    ...
    // Map existing value if present. If the value in DB doesn't match options,
    // it will just show no selection, which is fine.
    _selectedOption = profile?.drinking;
  }
  ...
  Future<void> _save() async {
    try {
      ...
      await repo.updateProfile(userId, {'drinking': _selectedOption});

      if (mounted) {
        final currentProfile = ref.read(currentUserProfileProvider).value;
        if (currentProfile != null) {
          final updatedProfile = currentProfile.copyWith(
            drinking: _selectedOption,
          );
          ref
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to update drinking habit: $e')),
        );
      }
    }
  }
