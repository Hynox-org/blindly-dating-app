...
class KidsPreferenceScreen extends ConsumerStatefulWidget {
  ...
  const KidsPreferenceScreen({super.key, this.isEditMode = false});

  @override
  ConsumerState<KidsPreferenceScreen> createState() =>
      _KidsPreferenceScreenState();
}
...
class _KidsPreferenceScreenState extends ConsumerState<KidsPreferenceScreen> {
  ...
  void initState() {
    ...
    _selectedOption = profile?.kidsPreference;
  }
  ...
  Future<void> _save() async {
    try {
      ...
      await repo.updateProfile(userId, {'kids_preference': _selectedOption});

      if (mounted) {
        final currentProfile = ref.read(currentUserProfileProvider).value;
        if (currentProfile != null) {
          final updatedProfile = currentProfile.copyWith(
            kidsPreference: _selectedOption,
          );
          ref
        ...
        children: [
          Expanded(
            child: ListView.separated(
              ...
              itemCount: _kidsOptions.length,
              separatorBuilder: (context, index) => const SizedBox(height: 12),
              itemBuilder: (context, index) {
                final option = _kidsOptions[index];
