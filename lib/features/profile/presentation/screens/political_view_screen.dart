...
class PoliticalViewScreen extends ConsumerStatefulWidget {
  ...
  const PoliticalViewScreen({super.key, this.isEditMode = false});

  @override
  ConsumerState<PoliticalViewScreen> createState() =>
      _PoliticalViewScreenState();
}
...
class _PoliticalViewScreenState extends ConsumerState<PoliticalViewScreen> {
  ...
  void initState() {
    ...
    _selectedOption = profile?.politics;
  }
  ...
  Future<void> _save() async {
    try {
      ...
      await repo.updateProfile(userId, {'politics': _selectedOption});

      if (mounted) {
        final currentProfile = ref.read(currentUserProfileProvider).value;
        if (currentProfile != null) {
          final updatedProfile = currentProfile.copyWith(
            politics: _selectedOption,
          );
          ref
        ...
        children: [
          Expanded(
            child: ListView.separated(
              ...
              itemCount: _politicalOptions.length,
              separatorBuilder: (context, index) => const SizedBox(height: 12),
              itemBuilder: (context, index) {
                final option = _politicalOptions[index];
