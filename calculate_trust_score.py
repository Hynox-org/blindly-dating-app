...
def get_db_connection():
    ...
    # Use the hardcoded URL provided by the user as a fallback
    database_url = os.environ.get("DATABASE_URL") or "postgresql://postgres.icvncmawahwbpiohrcxv:Blindly.Hynox@aws-1-ap-northeast-1.pooler.supabase.com:6543/postgres"
...
def lambda_handler(event, context):
    ...
    Handles different invocation styles (Direct, Function URL, API Gateway Proxy).
    """
    logger.info("Received event: %s", json.dumps(event))

    profile_id = None

    # 1. Try to extract profile_id from the event
    if isinstance(event, dict):
        # A. Direct invocation or console test
        profile_id = event.get("profile_id")

        # B. From HTTP body (Lambda Proxy / Function URL)
        if not profile_id and "body" in event and event["body"]:
            try:
                body = event["body"]
        ...
        # C. From query string parameters
        if not profile_id and "queryStringParameters" in event and event["queryStringParameters"]:
            profile_id = event["queryStringParameters"].get("profile_id")

    # Clean up the ID
    profile_id = (str(profile_id) if profile_id else "").strip()

    if not profile_id:
        logger.error("Validation failed: profile_id is missing")
        return {
            "statusCode": 400, 
            "body": json.dumps({"error": "profile_id is required", "received_event": event})
        }
